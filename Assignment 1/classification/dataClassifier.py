...
def enhancedFeatureExtractorDigit(datum):
	...
	def getImage():
		"""
		Helper Function to get the whole image in a numpy array
		"""
		image = []
		for i in range(DATUM_HEIGHT):
			temp =[]
			for j in range(DATUM_WIDTH):
				temp.append(getPixelVal(i, j))
			image.append(temp)
		return image

	def getArea():
		"""
		It returns the area of the shape in the image
		"""
		area = 0
		for i in range(DATUM_HEIGHT):
			for j in range(DATUM_WIDTH):
				area += getPixelVal(i,j)
		return area

	def getPerimeter():
		"""
		It returns the perimeter of the shape using 4-neighbour boundary
		"""
		count = 0
		image = np.array(getImage())
		for i in range(1, DATUM_WIDTH - 1):
			for j in range(1, DATUM_HEIGHT - 1):
				if (image[i-1][j] + image[i][j-1] + image[i+1][j] + image[i][j+1] != 4) and (image[i][j] == 1):
					count += 1
		return count

	def getCorners():
		"""
		It tries to return the number of corners using 8-neighbour boundary but fails to do so 
		"""
		count = 0
		image = np.array(getImage())
		for i in range(1, DATUM_WIDTH - 1):
			for j in range(1, DATUM_HEIGHT - 1):
				if (image[i-1][j-1] + image[i-1][j] + image[i-1][j+1] + image[i][j-1] + image[i][j+1] + image[i+1][j-1] + image[i+1][j] + image[i+1][j+1] <= 4) and (image[i][j] == 1):
					count +=1
		return count
		
	def getXGrad():
		"""
		It returns the absolute summation of all the x direction gradients
		"""
		count = 0
		image = np.array(getImage())
		for i in range(1, DATUM_WIDTH - 1):
			for j in range(1, DATUM_HEIGHT - 1):
				count += abs(image[i][j] - image[i][j+1])
		return count

	def getYGrad():
		"""
		It returns the absolute summation of all the y direction gradients
		"""
		count = 0
		image = np.array(getImage())
		for i in range(1, DATUM_WIDTH - 1):
			for j in range(1, DATUM_HEIGHT - 1):
				count += abs(image[i][j] - image[i+1][j])
		return count
 
	features = util.Counter()
	features[0] = (getArea()/getPerimeter())*100
	features[1] = getYGrad()*10
	return features
